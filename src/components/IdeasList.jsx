...
function IdeasList() {
  ...
  return (
    <>
      {scriptScreen === false ? (
        <div className="mt-12 flex flex-col gap-6 items-center justify-center px-4 sm:px-8">
          {ideas.map((each) => (
            <div
              key={each.id}
              className="relative w-full sm:w-[500px] bg-gradient-to-b from-slate-700/80 to-gray-800/90 rounded-3xl p-6 text-white shadow-lg shadow-indigo-500/40 backdrop-blur-lg flex items-center justify-between transition-all duration-300 ease-in-out hover:scale-105"
            >
        ...
        <div className="flex flex-col items-center justify-center px-4 sm:px-8">
          <h1 className="text-2xl sm:text-3xl text-black mt-4 mb-2">IDEA : {text}</h1>

          <textarea
            value={inputScript}
            onChange={(e) => setInputScript(e.target.value)}
            className="w-full sm:w-[600px] h-[300px] bg-black text-white rounded-lg p-4 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900 outline-none resize-none overflow-y-scroll"
