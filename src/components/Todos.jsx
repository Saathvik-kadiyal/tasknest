    ...
    <>
      {scriptScreen == false ? (
        <ul className="list-none flex flex-wrap flex-col items-center justify-center mt-12 gap-4">
          {todos.map((todo) => (
            <li
              className="w-full sm:w-[500px] bg-gradient-to-b from-slate-700/80 to-gray-800/90 rounded-3xl p-6 text-white shadow-lg shadow-indigo-500/40 backdrop-blur-lg flex items-center justify-between transition-all duration-300 ease-in-out hover:scale-105"
              key={todo.id}
            >
              {editId === todo.id ? (
                <form
